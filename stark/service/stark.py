...
class Option(object):
    ...
    def get_queryset(self, _field, model_class, query_dict):
        if isinstance(_field, ForeignKey) or isinstance(_field, ManyToManyField):
            row = Row(_field.related_model.model.objects.filter(**self.condition), self, query_dict)
        else:
            if self.is_choice:
...
class StarkConfig(object):
    ...
    def add_view(self, request):
        ...
        form = AddModelForm(data=request.POST, files=request.FILES)
        if form.is_valid():
    ...
    def change_view(self, request, pk):
        ...
        if request.method == 'GET':
            ...
            return render(request, 'stark/change.html', {'form': form})
        form = ModelFormClass(data=request.POST, instance=obj, files=request.FILES)
        if form.is_valid():
            self.save(form)
            return redirect(self.reverse_list_url())
        return render(request, 'stark/change.html', {'form': form})
